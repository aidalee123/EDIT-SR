"""Inspect EditSR repair traces and print per-task results directly to stdout."""
...
def extract_body_token_ids(seq_like, pad_id: int, start_id: int, finish_id: int):
    seq = trim_token_sequence(seq_like, pad_id=pad_id, finish_id=finish_id, include_finish=False)
    if len(seq) > 0 and int(seq[0]) == int(start_id):
        seq = seq[1:]
    return [int(v) for v in seq if int(v) != int(pad_id)]



def infer_editor_output_from_transition(prev_seq_like, next_seq_like, selected_edit, pad_id: int, start_id: int, finish_id: int):
    if selected_edit is None:
        return []

    prev_body = extract_body_token_ids(prev_seq_like, pad_id=pad_id, start_id=start_id, finish_id=finish_id)
    next_body = extract_body_token_ids(next_seq_like, pad_id=pad_id, start_id=start_id, finish_id=finish_id)

    span_start = int(selected_edit.get('span_start', selected_edit.get('root_idx', -1)))
    span_end = int(selected_edit.get('span_end', span_start + 1))
    if span_start < 0 or span_start > len(next_body):
        return []

    prev_suffix = prev_body[span_end:] if 0 <= span_end <= len(prev_body) else []
    if len(prev_suffix) == 0:
        next_end = len(next_body)
    else:
        next_end = len(next_body) - len(prev_suffix)
        if next_end < span_start or next_body[next_end:] != prev_suffix:
            matched = False
            for cand_end in range(len(next_body), span_start - 1, -1):
                if next_body[cand_end:] == prev_suffix:
                    next_end = cand_end
                    matched = True
                    break
            if not matched:
    ...
    return [int(v) for v in next_body[span_start:next_end]]



def build_step_edit_summary(prev_seq_like, next_seq_like, selected_edit, pad_id: int, start_id: int, finish_id: int, id2word: dict):
    if selected_edit is None:
        return {
            'tagger_position': -1,
            'tagger_action_id': -1,
            'tagger_action_name': '',
            'editor_output_token_ids': [],
            'editor_output_tokens': [],
        }

    action_id = int(selected_edit.get('action', -1))
    editor_output_ids = infer_editor_output_from_transition(
        prev_seq_like=prev_seq_like,
        next_seq_like=next_seq_like,
    ...
    return {
        'tagger_position': int(selected_edit.get('root_idx', -1)),
        'tagger_action_id': action_id,
        'tagger_action_name': TAGGER_ACTION_NAME_BY_ID.get(action_id, f'ACT_{action_id}'),
        'editor_output_token_ids': editor_output_ids,
        'editor_output_tokens': token_ids_to_words(editor_output_ids, id2word),
    }
...
def empty_repair_trace_dict(max_iters: int):
    ...
    for i in range(1, max_iters + 1):
        ...
        out[f'repair_iter_{i}_tagger_positions'] = json_dumps_safe([])
        out[f'repair_iter_{i}_tagger_action_ids'] = json_dumps_safe([])
        out[f'repair_iter_{i}_tagger_action_names'] = json_dumps_safe([])
        out[f'repair_iter_{i}_editor_output_token_ids'] = json_dumps_safe([])
        out[f'repair_iter_{i}_editor_output_tokens'] = json_dumps_safe([])
    return out
...
def collect_repair_trace_stats(
    ...
    for step_idx in range(max_iters):
        ...
        step_state_tokens = []
        step_tagger_positions = []
        step_tagger_action_ids = []
        step_tagger_action_names = []
        step_editor_output_token_ids = []
        step_editor_output_tokens = []
        step_neds = []
        next_active_states = []
        ...
        for state in active_states:
            ...
            step_edit_summary = build_step_edit_summary(
                prev_seq_like=state['tokens'][0].detach().cpu(),
                next_seq_like=tok,
                selected_edit=succ.get('selected_edit', None),
                pad_id=pad_id,
                start_id=start_id,
                finish_id=finish_id,
                id2word=id2word,
            )
            step_tagger_positions.append(int(step_edit_summary['tagger_position']))
            step_tagger_action_ids.append(int(step_edit_summary['tagger_action_id']))
            step_tagger_action_names.append(str(step_edit_summary['tagger_action_name']))
            step_editor_output_token_ids.append([int(v) for v in step_edit_summary['editor_output_token_ids']])
            step_editor_output_tokens.append([str(v) for v in step_edit_summary['editor_output_tokens']])
        ...
        trace[f'repair_iter_{iter_no}_tagger_positions'] = json_dumps_safe(step_tagger_positions)
        trace[f'repair_iter_{iter_no}_tagger_action_ids'] = json_dumps_safe(step_tagger_action_ids)
        trace[f'repair_iter_{iter_no}_tagger_action_names'] = json_dumps_safe(step_tagger_action_names)
        trace[f'repair_iter_{iter_no}_editor_output_token_ids'] = json_dumps_safe(step_editor_output_token_ids)
        trace[f'repair_iter_{iter_no}_editor_output_tokens'] = json_dumps_safe(step_editor_output_tokens)
...
def result_columns(max_repair_iters: int):
    ...
    for i in range(1, max_repair_iters + 1):
        cols.extend([
            ...
            f'repair_iter_{i}_tagger_positions',
            f'repair_iter_{i}_tagger_action_ids',
            f'repair_iter_{i}_tagger_action_names',
            f'repair_iter_{i}_editor_output_token_ids',
            f'repair_iter_{i}_editor_output_tokens',
        ])
    return cols
...
def main(cfg):
    ...
    print("=== 使用 table3_with_n200.csv 进行测试 ===")
    ...
    benchmarks_path = resolve_first_existing_path([
        str(resolve_path("table3_with_n200.csv", base="scripts")),
        str(resolve_path("scripts/table3_with_n200.csv", base="project")),
        str(resolve_path("data/table3_with_n200.csv", base="scripts")),
        str(scripts_path("table3_with_n200.csv")),
    ])
    ...
    try:
        for row_idx, row in benchmarks.iterrows():
            name = str(row["name"])
            # if name in ["Koza-1, Nguyen-2", "Koza-2", "Koza-3", "Nguyen-1", "Nguyen-3", "Nguyen-4", "Nguyen-5",
            #              "Nguyen-6", "Nguyen-7", "Nguyen-8", "Nguyen-9", "Nguyen-10", "Nguyen-11", "Nguyen-12",
            #             "Pagie-1", "Korns-1", "Korns-2", "Korns-3","Korns-4", "Korns-5", "Korns-6","Korns-7", "Korns-8", "Korns-9",
            #             "Korns-10", "Korns-11", "Korns-12","Korns-13", "Korns-14", "Korns-15"]:
            #     continue
            print("#####################################################################################")
            print(f"Task: {name}")
            ...
            for rep in range(REPEATS):
                ...
                try:
                    ...
                    variables = [f"x_{i}" for i in range(1, num_var + 1)]
                    print("Expr:", expr)
                    ...
                    X_raw, y_raw, _ = sample_points_from_specs(
                        lam, num_vars=num_var, specs=train_specs, target_noise=noise, seed=seed_train
                    )
                    X_test_raw, _, y_test_raw = sample_points_from_specs(
                    ...
                    for beam in BEAMS:
                        try:
                            current_scaler_type = 'auto' if (beam == 101 and BEAM50_FORCE_AUTO_SCALER) else SCALER_TYPE
                    ...
                    print(
                        ...
                        f"SR={int(main_sr)} | Complexity={main_complexity} | Time={main_time:.2f}s"
                    )

                    del X_raw, y_raw, X_test_raw, y_test_raw, X_tensor, y_tensor
                    gc.collect()
